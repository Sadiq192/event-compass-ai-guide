...
const EventDetailsPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchEventAndTasks = async () => {
      ...
      try {
        ...
        const tasksData: BackendTask[] = await tasksResponse.json(); // Use BackendTask[] here
        setTasks(tasksData.map(task => ({ ...task, id: task._id }))); // Map _id to id for tasks
      } catch (error) {
        console.error('Error fetching event or tasks:', error);
        toast.error('Failed to load event details or tasks.');
      } finally {
        setLoading(false);
      }
    };
  ...
  return <EventDetailsComponent {...event} tasks={tasks} />;
};
